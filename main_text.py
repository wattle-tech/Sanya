...
def processing(text):
    ...
    if max_t > max_tr:
        ...
        print("Функция разговора в разработке")
...
def translate_df():
    print('Ожидайте функцию в последующих обновлениях')
...
def to_epoch(text: str): #Перевод строки в Unix Epoch
    ...
    try:
        ...
        print("что-то пошло не так, запустите команду заново!")
...
def timer_time_to_epoch(time: str):
    if time.endswith("минут"):
        ...
        if time[0:2] > 24:
            print("Не возможно поставить таймер более чем на 24 часа!")
        else:
            time = time[0:2]
            time = time * 60 * 60
            return time
...
def add_alarm_clock():
    print("Когда хотите, что бы прозвенел будильник?")
    time = input
    date = to_epoch(time)
    clock.add("Будильник", date) #создаём будильник 
    print("Будильник добавлен")
